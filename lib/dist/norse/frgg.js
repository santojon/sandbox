...
function Frgg(views, container, options) {
    ...
    f.prototype = {
        ...
        loadPage(page, appContainer) {
            ...
            if (appContainer) {
                if (options.inPage) {
                    appContainer.innerHTML = setup(container.inPageMappings[page])
                } else {
                    appContainer.innerHTML = load(mappings[page]) || ''
                }
            }
        }
    }
    ...
    compileTemplates = function () {
        ...
        for (i = 0; i < scripts.length; i++) {
            if (scripts[i].type == 'text/frigga') {
                ...
                if (scripts[i].src) {
                    // get page name
                    var mapName = scripts[i].src.split(/\_/)[1].split(/\./)[0];
                    mapName = mapName.charAt(0).toUpperCase() + mapName.substring(1);

                    // add adta to mappings and view
                    src[mapName] = load(scripts[i].src);
                    elem.innerHTML = src[mapName] || elem.innerHTML;
                } else {
